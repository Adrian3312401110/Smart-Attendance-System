...
@app.get("/server-waktu")
def server_waktu():
    """
    Dipakai frontend untuk sinkronisasi jam. Kalau jam di laptop pengguna
    tidak sama persis dengan server, perhitungan "sudah waktunya absen atau
    belum" di sisi klien bisa meleset -> ini penyebab paling umum dari bug
    "belum waktunya absensi padahal sudah waktunya" / "absensi gagal" pada
    mode acak. Frontend akan menghitung selisih (offset) lalu memakainya.
    """
    return {"waktu_server": datetime.now().isoformat()}


@app.get("/absensi/status-hari-ini/{id_mahasiswa}")
def status_absensi_hari_ini(id_mahasiswa: str, db: Session = Depends(get_db)):
    """
    Mengembalikan daftar jadwal yang absensinya SUDAH final (selesai) untuk
    mahasiswa ini pada hari ini. Dipakai frontend untuk mengunci jadwal yang
    sudah diabsen supaya tidak bisa dibuka/diulang lagi hari itu.
    """
    now = datetime.now()
    hari_ini = now.date()
    hari_label = hari_ini_label()
    ...
    daftar = db.query(models.Absensi).filter(
        models.Absensi.id_mahasiswa == id_mahasiswa,
        func.date(models.Absensi.tanggal_absensi) == hari_ini,
    ).all()
        ...
        "data": [
            {
                ...
                "status": a.status,
                "waktu": a.tanggal_absensi.strftime("%H:%M") if a.tanggal_absensi else None,
            }
            for a in daftar
        ]
    }
...
def login_user(payload: LoginRequest, db: Session = Depends(get_db)):
    ...
    if not account:
        return JSONResponse(status_code=401, content={
            "berhasil": False,
            "alasan": "akun_tidak_ditemukan",
            "pesan": "Akun dengan email ini tidak ditemukan. Periksa kembali email Anda atau daftar akun baru.",
        })
    ...
    if not verify_password(payload.password, account.password_hash):
        return JSONResponse(status_code=401, content={
            ...
            "pesan": "Password yang Anda masukkan salah. Silakan coba lagi.",
        })
...
def gabung_kelas(data: GabungKelasRequest, db: Session = Depends(get_db)):
    ...
    if sudah_ada:
        # Kalau permintaan sebelumnya DITOLAK, izinkan mahasiswa mengajukan
        # ulang alih-alih diblokir selamanya.
        if sudah_ada.status == "rejected":
            sudah_ada.status = "pending"
            sudah_ada.bergabung_pada = datetime.now()
            db.commit()
            return {
                "berhasil": True,
                "pesan": f"Permintaan bergabung ke kelas {kelas.nama} terkirim ulang, menunggu persetujuan dosen"
            }
...
@app.put("/kelas/anggota/{id_anggota}/reject")
def reject_anggota(id_anggota: int, db: Session = Depends(get_db)):
    """
    Endpoint baru: tolak permintaan bergabung TANPA menghapus row-nya, supaya
    mahasiswa yang bersangkutan bisa melihat status "Ditolak" di halaman
    Gabung Kelas, dan bisa mengajukan ulang.
    """
    anggota = db.query(models.KelasMahasiswa).filter(models.KelasMahasiswa.id == id_anggota).first()
    if not anggota:
        return JSONResponse(status_code=404, content={"berhasil": False, "pesan": "Data tidak ditemukan"})
    ...
    db.commit()
    return {"berhasil": True, "pesan": "Permintaan mahasiswa berhasil ditolak"}
