...
function hariLabelUntuk(d: Date): string {
  return DAFTAR_HARI[d.getDay()];
}
...
// `now` WAJIB dikirim dari luar (hasil sinkronisasi dengan jam server) supaya
// tidak meleset kalau jam laptop pengguna tidak akurat.
...
function hitungStatusWaktu(jadwal: JadwalItem, now: Date): {
  bisa: boolean;
  hari_cocok: boolean;
  ...
  const hariSekarang = hariLabelUntuk(now);
...
export default function AmbilAbsensiPage() {
  ...
  // ===== Sinkronisasi jam dengan server (memperbaiki bug "belum waktunya" /
  // "gagal" padahal jam sudah tepat, akibat jam laptop pengguna tidak akurat) =====
  const serverOffsetRef = useRef(0);

  const sinkronWaktuServer = useCallback(async () => {
    try {
      const res = await fetch("http://localhost:8000/server-waktu");
      const data = await res.json();
      const waktuServer = new Date(data.waktu_server).getTime();
      serverOffsetRef.current = waktuServer - Date.now();
    } catch {
      // Kalau gagal, tetap pakai jam lokal apa adanya
    }
  }, []);

  const nowServer = useCallback((): Date => {
    return new Date(Date.now() + serverOffsetRef.current);
  }, []);

  useEffect(() => {
    sinkronWaktuServer();
    const timer = setInterval(sinkronWaktuServer, 5 * 60 * 1000); // sinkron ulang tiap 5 menit
    return () => clearInterval(timer);
  }, [sinkronWaktuServer]);

  // ===== Status "sudah absen hari ini" per jadwal — untuk mengunci jadwal
  // yang sudah selesai supaya tidak bisa dibuka ulang =====
  const [statusHariIni, setStatusHariIni] = useState<Record<number, StatusHariIni>>({});

  const muatStatusHariIni = useCallback(async () => {
    if (!user?.id) return;
    try {
      const res = await fetch(`http://localhost:8000/absensi/status-hari-ini/${user.id}`);
      const data = await res.json();
      const map: Record<number, StatusHariIni> = {};
      (data.data ?? []).forEach((d: { id_jadwal: number; status: string; waktu: string | null }) => {
        map[d.id_jadwal] = { status: d.status, waktu: d.waktu };
      });
      setStatusHariIni(map);
    } catch {
      // abaikan, tidak fatal
    }
  }, [user]);
  ...
  useEffect(() => {
    ...
    function periksaWaktu() {
      ...
      const now = nowServer();
      const sw = hitungStatusWaktu(jadwalTerpilih, now);
      setStatusWaktu(sw);
  ...
  function pilihJadwal(j: JadwalItem) {
    if (statusHariIni[j.id]) return; // sudah selesai diabsen hari ini, tidak bisa dibuka lagi
  ...
  // Kunci: kalau jadwal ini sudah selesai diabsen hari ini, tidak bisa mulai lagi
  if (jadwalTerpilih && statusHariIni[jadwalTerpilih.id]) {
    setResult({
      berhasil: false,
      pesan: "Anda sudah menyelesaikan absensi untuk jadwal ini hari ini. Sesi absensi sudah ditutup otomatis.",
    });
    return;
  }

  // Sinkronkan ulang jam server sesaat sebelum validasi, supaya seakurat mungkin
  await sinkronWaktuServer();

  // ===== Validasi hari + waktu di sisi klien sebelum memulai gesture =====
  if (jadwalTerpilih) {
    const sw = hitungStatusWaktu(jadwalTerpilih, nowServer());

    if (!sw.hari_cocok) {
      setResult({
        berhasil: false,
        pesan: `📅 Jadwal ini hanya berlaku pada hari ${jadwalTerpilih.hari}, bukan hari ini (${hariLabelUntuk(nowServer())}).`,
      });
      return;
    }
  ...
  try {
    ...
    if (data.selesai) {
      ...
      // Kalau absensi hari ini sudah LENGKAP, kunci jadwal ini otomatis
      // supaya tidak bisa dibuka/diulang lagi hari ini.
...
}, [selectedJadwal, user, muatStatusHariIni]);
  ...
  const sudahAbsenJadwalTerpilih = jadwalTerpilih ? !!statusHariIni[jadwalTerpilih.id] : false;
        ...
        <section className="flex-1 flex items-center justify-center p-8">
          <div className="w-full max-w-lg bg-card text-card-foreground border border-border rounded-2xl shadow p-8">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
              Pilih jadwal yang sesuai untuk memulai sesi absensi. Hanya jadwal dari kelas yang sudah Anda ikuti yang akan tampil di sini. Hari ini: <strong>{hariLabelUntuk(nowServer())}</strong>
            </p>
            ...
            <div className="space-y-3 max-h-[60vh] overflow-y-auto">
              ...
              {jadwalList.map((j) => {
                const swj = hitungStatusWaktu(j, nowServer());
                const sudahAbsen = !!statusHariIni[j.id];
                return (
                  <button
                    key={j.id}
                    onClick={() => pilihJadwal(j)}
                    disabled={!j.aktif || sudahAbsen}
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        {sudahAbsen && (
                          <span className="text-[10px] font-bold bg-blue-100 dark:bg-blue-950/50 text-blue-600 dark:text-blue-300 px-2 py-1 rounded-full">
                            ✓ Sudah Absen{statusHariIni[j.id]?.waktu ? ` (${statusHariIni[j.id].waktu})` : ""}
                          </span>
                        )}
                        {!sudahAbsen && !j.aktif && (
                          <span className="text-[10px] font-bold bg-red-100 text-red-500 px-2 py-1 rounded-full">Nonaktif</span>
                        )}
                        {!sudahAbsen && j.aktif && !swj.hari_cocok && (
  ...
  return (
    <main className="min-h-screen bg-background text-foreground flex">
      ...
      <section className="flex-1 p-8">
        ...
        {/* Banner: jadwal ini sudah selesai diabsen hari ini -> otomatis tertutup */}
        {sudahAbsenJadwalTerpilih && (
          <div className="mb-4 rounded-xl border border-blue-200 dark:border-blue-900 bg-blue-50 dark:bg-blue-950/30 px-5 py-3 flex items-center gap-3">
            <span className="text-xl">✅</span>
            <div>
              <p className="font-semibold text-blue-700 dark:text-blue-300 text-sm">
                Anda sudah absen hari ini{statusHariIni[jadwalTerpilih!.id]?.waktu ? ` (${statusHariIni[jadwalTerpilih!.id].waktu})` : ""}
              </p>
              <p className="text-xs text-blue-500 dark:text-blue-400">
                Sesi absensi untuk jadwal ini sudah ditutup otomatis dan tidak bisa dibuka lagi hari ini.
              </p>
            </div>
          </div>
        )}
        ...
        {statusWaktu && !statusWaktu.hari_cocok && jadwalTerpilih && !sudahAbsenJadwalTerpilih && (
          <div className="mb-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-5 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Jadwal <strong>{jadwalTerpilih.id_mata_kuliah}</strong> hanya berlaku pada hari <strong>{jadwalTerpilih.hari}</strong>. Hari ini adalah <strong>{hariLabelUntuk(nowServer())}</strong>.
              </p>
            </div>
          </div>
        )}
        ...
        {!sudahAbsenJadwalTerpilih && statusWaktu?.hari_cocok && !statusWaktu.bisa && statusWaktu.sudah_lewat && (
  <div className="mb-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-5 py-3 flex items-center gap-3">
    <span className="text-xl">🔒</span>
    <div>
      ...
      <p className="text-xs text-slate-500 dark:text-slate-400">Jendela absensi untuk jadwal ini hari ini sudah berakhir.</p>
        ...
        {!sudahAbsenJadwalTerpilih && statusWaktu?.hari_cocok && statusWaktu?.bisa && (
          ...
          <div className="col-span-2 bg-card text-card-foreground border border-border rounded-2xl shadow p-6">
            ...
            <div className="mt-4 flex gap-3">
              <button
                ...
                disabled={(tahapan !== "idle" && tahapan !== "selesai") || sudahAbsenJadwalTerpilih}
              ...
              <button
                onClick={mulaiSesi}
                disabled={!cameraOn || !selectedJadwal || (tahapan !== "idle" && tahapan !== "selesai") || sudahAbsenJadwalTerpilih}
                className="rounded-xl bg-blue-600 px-5 py-2 text-sm font-bold text-white disabled:opacity-50"
              >
                {sudahAbsenJadwalTerpilih
                  ? "Sudah Absen Hari Ini"
                  : tahapan === "idle" || tahapan === "selesai"
                    ? "Mulai Absensi"
                    : "Sedang Berjalan..."}
              </button>
            </div>

            {/* ===== Info hasil absensi — dipindah ke sini, tepat di bawah tombol,
                 supaya pengguna tidak perlu scroll ke bawah halaman. ===== */}

            {/* Hasil sesi PARSIAL — sesi ini beres, tapi masih ada sesi lain hari ini */}
            {result && result.hariSelesai === false && (
              <div className="mt-4 rounded-2xl border border-blue-200 dark:border-blue-900 bg-blue-50 dark:bg-blue-950/30 p-5">
                <span className="rounded-full px-3 py-1 text-xs font-bold bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300">
                  Sesi Tercatat
                </span>
                <h2 className="mt-3 text-lg font-bold">{result.pesan}</h2>
              ...
              <div className={`mt-4 rounded-2xl border p-5 ${
                ...
                {result.berhasil && (
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-3">
                    Jadwal ini sudah otomatis ditutup dan tidak bisa diabsen lagi hari ini.
                  </p>
                )}

                {!result.berhasil && !sudahAbsenJadwalTerpilih && (
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
          ...
          <aside className="space-y-6">
            <div className="bg-card text-card-foreground border border-border rounded-2xl shadow p-6">
              ...
              <StatusRow
                label="Sesi"
                value={
                  sudahAbsenJadwalTerpilih ? "Sudah Absen" :
                ...
                green={tahapan === "selesai" || sudahAbsenJadwalTerpilih}
              />
              <StatusRow
                label="Waktu Absensi"
                value={
                  sudahAbsenJadwalTerpilih ? "Ditutup" :
                  !statusWaktu ? "-" :
                ...
                green={!sudahAbsenJadwalTerpilih && statusWaktu?.hari_cocok === true && (statusWaktu?.bisa ?? false)}
              />
            </div>
                  ...
                  <>
                    <div className="flex flex-wrap gap-2">
                      {jadwalTerpilih.daftar_jam_absensi.map((jam) => {
                        const now = nowServer();
                        const hariCocok = jadwalTerpilih.hari === hariLabelUntuk(now);
                        const nowMenit = now.getHours() * 60 + now.getMinutes();
