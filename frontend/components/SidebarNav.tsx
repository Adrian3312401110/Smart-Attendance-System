...
export default function SidebarNav({ role }: SidebarNavProps) {
  ...
  // "Pengaturan Pengguna" untuk mahasiswa dipindah keluar dari menu utama,
  // ditempatkan di bagian bawah (lihat render di bawah), tepat di atas tombol
  // mode terang/gelap.
  const menu =
    role === "dosen"
      ? [
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 space-y-1.5 pt-4 border-t border-white/5">
        {/* Pengaturan Pengguna (mahasiswa) — diletakkan di atas tombol mode terang/gelap */}
        {role === "mahasiswa" && (
          <Link
